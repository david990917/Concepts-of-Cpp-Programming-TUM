...
/// token
[[nodiscard]] State transition(state::Start, Token token);

/// Transition from the `Valid` state to the next state depending on the given
/// token
[[nodiscard]] State transition(state::Valid, Token token);

/// Transition from the `Invalid` state to the next state depending on the given
/// token
[[nodiscard]] State transition(state::Invalid, Token token);
...
[[nodiscard]] bool is_valid_sql_query(std::vector<Token> tokens);
}   // namespace sql
